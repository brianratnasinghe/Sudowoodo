...
import argparse, shutil, os, re, random, textwrap
...
def parse_epsilon_map(epsilon_str):
    """
    Parse CC=1.0,CX=0.8,CP=0.7,XX=0.6,XP=0.5,PP=0.4
    to {('C','C'): 1.0, ...}
    """
    mapping = {}
    for item in epsilon_str.split(','):
        ...
        key = key.strip().upper()
        val = float(val)
        # Map CC, CX, CP, XX, XP, PP to (C,C), (C,X), etc.
        if key in ['CC','CX','CP','XX','XP','PP']:
            mapping[(key[0], key[1])] = val
            mapping[(key[1], key[0])] = val  # symmetry
    return mapping

def scale_epsilon_in_itp(itp_path, new_path, epsilon_map):
    """
    For each [ nonbond_params ] line, set epsilon from epsilon_map if present.
    ...
    for line in lines:
        ...
        if in_nb and re_lj.match(line):
            ...
            # Use epsilon_map if specified for (i, j)
...
# --- RANDOMIZE POLYMER STRUCTURE ---
def randomize_structures(seed, out_dir):
    # For demo: just copy the starting gro files (real randomization would be more complex)
    for src, dst in [
        ('X.gro', out_dir / 'X.gro'),
        ('P.gro', out_dir / 'P.gro'),
        ('C.gro', out_dir / 'C.gro')
...
def generate_itps(args, out_dir, epsilon_map):
    ...
    # Xylo, Pctn, Cellulose: just copy (no epsilon in these files)
    for src, dst in [
